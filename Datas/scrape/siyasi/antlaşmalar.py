...
url = f"https://tr.wikipedia.org/wiki/Türkiye%27nin_antlaşmaları_listesi"
...
mainTable = soup.find("table", {"class": "wikitable sortable jquery-tablesorter"})
tbody = mainTable.find("tbody")
trs = tbody.find_all("tr")
for tr in trs:
    td = tr.find("td")
    if td and td.find("a") and td.find("a").get("href"):
        links.append("https://tr.wikipedia.org" + td.find("a").get("href"))
...
df.to_csv("antlaşmalar.csv", index=False, encoding="utf-8-sig")
