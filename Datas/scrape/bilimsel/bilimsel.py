...
url = f"https://tr.wikipedia.org/wiki/Türkiye%27de_bilim_ve_teknoloji"
...
lis = []
mainTable = soup.find("div", {"id": "mw-content-text"})
mainTable = mainTable.find_all("ul")
for i in mainTable:
    a_s = i.find_all("li")
    for a in a_s:
...
for link in lis:
    try:
        ...
        dictionary["category"].append("Bilimsel")
...
df.to_csv("aaaaa.csv", index=False, encoding="utf-8-sig")
