...
url = f"https://tr.wikipedia.org/wiki/Türkiye%27deki_intihar_saldırıları_listesi"
...
table = soup.find("table",class_="sortable wikitable plainrowheaders jquery-tablesorter")
tbody = table.find("tbody")
trs = tbody.find_all("tr")
for tr in trs:
    td = tr.find_all("td")[0]
    link = "https://tr.wikipedia.org" + td.find("a")["href"].strip() if td.find("a") else ""
    links.append(link)
...
df.to_csv("saldırılar.csv", index=False, encoding="utf-8-sig")
